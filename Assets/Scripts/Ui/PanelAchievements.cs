...
namespace Ui
{
	public class PanelAchievements : AahMonoBehaviour
	{
		...
		private void InitStats()
		{
			...
			{
               // print("statWidgetPrefab - "+statWidgetPrefab.statName);
				StatWidget statWidget = this.stats[i];
		...
		public void OnPanelOpened(Simulator simulator)
		{
			...
			for (int i = 0; i < this.stats.Count; i++)
			{
				...
				if (i < count)
				{
                    PlayerStat playerStat1 = activeStats[i];
                    if (LM.Get(playerStat1.nameKey) == "Dragons caught" || LM.Get(playerStat1.nameKey) == "Side quests completed" || LM.Get(playerStat1.nameKey) == "Side quests skipped")
                    {
                        //DestroyImmediate(statWidget.gameObject);

                        statWidget.transform.SetScale(0);
                    }
                    else
                    {
                    ...
                    //if (statWidget.statName.text == "Dragons caught" || statWidget.statName.text == "Side quests completed" ||statWidget.statName.text == "Side quests skipped")
                    //{
                    //    DestroyImmediate(statWidget.gameObject);

                    //    //statWidget.transform.SetScale(0);
                    //}
                    print("statWidgetPrefab - "+statWidget.statName.text);
                    num++;
                    }
