...
namespace Ui
{
    ...
    void UpdateCurrency()
        ...
        private void FixedUpdate()
        {
            ...
            //rint("eon here -"+ this.sim.GetCurrency(CurrencyType.AEON));
        ...
        {
            this.sim = sim;
            //this.sim.ConsumeCurrency(CurrencyType.GEM, 99);
            this.sim.GainCurrency(CurrencyType.GEM, 1);
...
//GC.Collect();
                ...
                while (i < num2)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        if (difference.Hours >= 4 )
                        {
                            ...
                            merchantItem.textPrice.text = "Claim";
        ...
        public void UpdateMine(float dt)
        {
            ...
            if (UiManager.stateJustChanged)
            {
                ...
                this.panelMine.currentLevel.transform.localPosition = new Vector3(1.0f, 200f, 1.0f);
                this.panelMine.currentLevel.rectTransform.sizeDelta = new Vector2(560f, 360f);

                if (!this.panelMine.dontUpdateLevel)
                {
                    this.panelMine.currentLevel.text = "\n \n \n"+string.Format(LM.Get("UI_LEVEL"), this.panelMine.selected.level + 1 + "\n \n <color=green>The timer for the token and scrap mine is common. You can collect only one at a time. So choose wisely!</color>");
                }
                else
                    this.panelMine.currentLevel.text = "\n \n <color=blue>The timer for the token and scrap mine is common. You can collect only one at a time. So choose wisely!</color>";
            ...
            //// Display the time in whatever format you prefer
            //string formattedTime = string.Format("{0:D2}:{1:D2}:{2:D2}",
            //    timeSpan.Hours, timeSpan.Minutes, timeSpan.Seconds);

            //Debug.Log("Time remaining: " + formattedTime);
       ...
       // }
                ...
                {
                    //this.panelMine.buttonMineCollect.text.text = formattedTime;//"Wait";//LM.Get("UI_SHOP_CHEST_0_WAIT");
                }
           ...
           // {
            ...
            else
            {
                ...
                //else
                //{
                //    this.panelMine.buttonMineCollect.text.text = "Wait";//LM.Get("UI_SHOP_CHEST_0_WAIT");
                //}
                //this.panelMine.isCd = true;
            }
        }
        ...
        private void Awake()
        {
            //this.sim.GainCurrency(CurrencyType.AEON, 1000);
            totalTimeInSeconds = 1 * 60 * 60;
            ...
            if (PlayerPrefs.HasKey(PlayerPrefsKey))
            {
                if (PlayerPrefs.GetString(PlayerPrefsKey) != "")
                    ...
                    PlayerPrefs.SetString(PlayerPrefsKey, DateTime.UtcNow.ToString());
            }
            else
                PlayerPrefs.SetString(PlayerPrefsKey, DateTime.UtcNow.ToString());

            print("save PlayerPrefsKey- " + PlayerPrefsKey);
            callONStartMine();
        }
        public void callONStartMine()
        {
            DateTime startTime = GetStartTime();
           
            TimeSpan elapsedTime = DateTime.UtcNow - startTime;
            float remainingTime = Mathf.Max(totalTimeInSeconds - (float)elapsedTime.TotalSeconds, 0f);
            UpdateTimerDisplay(remainingTime);
        }
        private DateTime GetStartTime()
        {
            print("11 PlayerPrefsKey- " + PlayerPrefsKey);

            string startTimeString = PlayerPrefs.GetString(PlayerPrefsKey);
            print("22 PlayerPrefsKey- "+startTimeString);
        ...
        private float GetRemainingTime()
        {
            ...
            return Mathf.Max(totalTimeInSeconds - (float)elapsedTime.TotalSeconds, 0f);
        }

        private bool IsTimerExpired()
        {
            //print("startTime - " + GetRemainingTime());
        ...
        private void UpdateTimerDisplay(float remainingTime)
        {
             ...
             string formattedTime = string.Format("{0:D2}:{1:D2}:{2:D2}",
             timeSpan.Hours, timeSpan.Minutes, timeSpan.Seconds);
            this.panelMine.buttonMineCollect.text.text = formattedTime;
           // Debug.Log("Time remaining: " + formattedTime);
        }
        public float totalTimeInSeconds =1*60*60; // 4 hours in seconds
        ...
        private void OnClickedMine(Mine mine)
        {
            ...
            if (mine is MineToken)
            {
                ...
                currentCurrancy = CurrencyType.TOKEN;
            }
            else
            {
                if (!(mine is MineScrap))
                ...
                currentCurrancy = CurrencyType.SCRAP;
            }
            this.panelMine.imageIcon.SetNativeSize();
            this.state = UiState.SHOP_MINE;
            UiManager.sounds.Add(new SoundEventUiSimple(SoundArchieve.inst.uiLootpackTap, 1f));
        }
        CurrencyType currentCurrancy;
        private void OnClickedMineCollect()
        {
            print("currentCurrancy - "+ currentCurrancy + this.panelMine.selected.GetReward());
            this.sim.GainCurrency(currentCurrancy, this.panelMine.selected.GetReward());
            totalTimeInSeconds =1*60* 60;
            PlayerPrefs.SetString(PlayerPrefsKey, DateTime.UtcNow.ToString());
            float num = 0.5f;
